...
| `mineru/ops/static/ops.js` | 总览计数改用服务端聚合;详情失败不再死循环;`clearTaskSelection()`;「终止任务」按钮;缓存提示条;`TASK_TERMINAL_STATES` / `isTaskTerminal()` 统一终态判据 |
| `tests/unit/test_task_cancel.py` | 新增:API 取消语义 + Router 转发/保留/404 语义 |
| `tests/unit/test_ops_console.py` | 新增:slim payload、SQL 计数、控制台 cancel、SSE 收尾、静态资源约定、终态判据共用 |
| `docker/multi/OPS_CONSOLE.md` | 新增「任务终止与缓存清理」小节,说明两个操作的差别 |
| `docker/multi/现场部署指南.md` | 日常操作里补一条「终止任务」说明 |
...
**终态判据统一(复审发现的遗漏)**
- 首轮提交把「终止任务」的显隐绑在“有没有产物”的判据上,导致**失败任务也显示「终止任务」**。
  实测:筛选「失败」的缓存任务,详情面板同时出现「终止任务」和「删除缓存」。
- 该按钮对终态任务有害:Router 对终态任务不转发上游,只丢本地记录;对 Router 已无记录的任务,
  控制台拿到 404 就报 502,用户看到的是“取消失败”,而实际效果与「删除缓存」相同。
- 改为新增 `TASK_TERMINAL_STATES` / `isTaskTerminal()`,让取消按钮显隐、详情 SSE 收尾、SSE 数据收尾
  三处共用同一套终态集合(原先三处各写了一份字面量数组,已统一)。
- 复测:失败任务 `hasCancel=false`、`hasDelete=true`;已完成 `hasCancel=false`;
  处理中 `hasCancel=true` 且点击后上游任务消失(`upstreamStillRunning=[]`)、Router 记录已清(`routerGone=true`)。
- 文档补上「终止任务」与「删除缓存」的区别:`OPS_CONSOLE.md` 新增「任务终止与缓存清理」小节,
  `现场部署指南.md` 日常操作里加一条指针。
...
- 「删除缓存」对批次任务不显示(`source_batch_run_id` 存在时不渲染按钮),这类任务要在批次页删除。
  这是既有设计,不是本轮改动。
- 「已取消 / 已中断」这两个状态只在任务被取消后由上游回传时可能出现;控制台当前的取消流程是
  确认后直接清掉本地记录,所以现场通常看不到这两个徽标。

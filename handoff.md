...
## 2026-09-23：运维控制台打磨（OPS_VERSION 2.0.1）

部署在 172.16.100.26:19000 的单 API 节点上逐页看过后改的。只动静态前端，没有新增接口，批量删除复用已有的 `DELETE /api/tasks/{task_id}`。

### 本轮完成

1. **任务列表可批量删除**。每行加勾选框，表头加全选框，工具栏显示「已选择 N 条」和「批量删除」。删除只清控制台自己的记录，确认框写明了这一点；来自批量测试的任务后端返回 409，结果里单独列出没删成的和原因。未结束的任务行上另有「停止」按钮，走 `POST /api/tasks/{id}/cancel`。
2. **配置中心默认折叠**。六个配置分组和「最终有效配置」的每一项都改成 `<details>`，默认收起，只有存在未生效或冲突的项才自动展开。「操作审计」改成折叠块，展开后才请求 `/api/audit`。
3. **总览只留正在发生的事**。「最近批量测试」整块从总览移除（批次记录只在「批量测试」页看，那里本来就有勾选和批量删除）；没有进行中的任务时显示空状态，不再用最近完成的任务填充。删掉永远显示「无数据」的「GPU 显存负载」指标卡，以及服务卡上没有显存数据时的「显存 无数据」行。失败数改为红色。
4. **配置页不再自己往上跳**。配置视图每 10 秒重拉一次最终有效配置，原来每次都整段替换 `#config-application-status` 的 `innerHTML`，页面高度骤变后滚动位置被夹紧，看起来像页面在自动刷新。现在渲染结果与上次相同时不碰 DOM，并去掉了 HTML 里每次都变的「检查时间」，让轮询在内容没变时不产生任何重绘。
...
mineru/cli/ops.py            # OPS_VERSION 2.0.0 -> 2.0.1，用于刷新静态资源缓存
handoff.md
```
...
# 部署到 172.16.100.26 后用浏览器逐页核对：
# 总览无 GPU 卡片、无批量测试表；任务页有全选与批量删除；
# 配置中心默认折叠，快照从 1200+ 行降到 237 行；控制台无报错。
```

未跑 pytest。本轮没有改 Python 行为，只改了 `OPS_VERSION` 常量。

### 部署说明

代码镜像标签 `mineru-code:v3.4.2-ops-ui24`，`docker/multi/env.multi` 的 `MINERU_CODE_IMAGE` 已指向它。

更新代码的顺序：先 `--force-recreate mineru-code-sync`，**等它变成 `Exited (0)` 再重启 api/router/ops**。它的 CMD 是把代码拷进共享卷，容器还在 `Up` 时就重启业务容器，卷里还是旧代码。这次就踩过一次：`ps` 显示 code-sync 仍在用 `ops-ui23`，是因为 `env.multi` 没改标签，compose 读的是那个值而不是本地镜像。

`OPS_VERSION` 没变的情况下浏览器会继续用缓存的 `ops.js`，部署后需要 Ctrl+Shift+R 强制刷新。

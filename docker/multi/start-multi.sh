...
case "${1:-help}" in
    ...
    start)
        echo "启动全部服务（Router + 3 个 API；VLM 为外部服务）..."
        ...
        echo "VLM 地址请查看 env.multi 中的 VLM_1/2/3_IP 和 VLM_1/2/3_PORT。"
        ;;
    ...
    *)
        ...
        echo ""
        echo "  build  - 构建镜像"
        echo "  start  - 启动全部（Router + 3 个 API）"
        echo "  stop   - 停止全部"
        echo "  status - 查看状态"
        echo "  logs   - 查看日志"
        echo ""
        ...
        echo ""
        echo "架构："
        echo "  Router (:8002) → API-1 + API-2 + API-3 → 外部 VLM-1/2/3"
        exit 1
        ;;
esac

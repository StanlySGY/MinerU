...
任务详情面板上有两个都会让任务从列表消失的操作,含义不同:

| 操作 | 作用范围 | 适用状态 |
| --- | --- | --- |
| 终止任务 | 控制台 → Router → 承接该任务的 API 全部停止解析,并清掉控制台快照 | 仅未结束的任务(等待中/处理中) |
| 删除缓存 | 只删控制台 SQLite 里的那条快照,任务在 Router 和 API 里照常运行 | 已结束的任务;批次任务不显示,需在批次页删除 |

- 「终止任务」不可撤销:正在解析的页会被中断,已完成页的产物不保留。点击后控制台先请求
  Router,Router 确认后才清缓存;Router 报错(例如上游 API 已不可达)时控制台保留记录并提示
  失败,不会显示一个其实没停的「已取消」。
- 「删除缓存」是只读操作的安全版本:任务本身不受影响,适合清理已经结束、只是占着列表的
  历史记录。如果某个任务在 Router 已不存在而控制台还有缓存,详情面板会提示「Router 已无
  此任务记录,当前显示的是控制台缓存」。
